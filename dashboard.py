...
# Allow CORS (for future React frontend)
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
...
def close_all_trades():
    try:
        ...
        for position in open_positions:
            ...
            # ✅ دریافت قیمت لحظه‌ای از جدول کندل‌ها
            cursor.execute("""
            ...
            """, (
                ...
                entry_price,
                exit_price,
                0.0,  # confidence برای فروش دستی
                datetime.utcnow().isoformat(),
                "simulation",
                profit
            ))

            # ✅ حذف پوزیشن از جدول open_trades
            cursor.execute("DELETE FROM open_trades WHERE symbol = %s", (symbol,))

            # ✅ بروزرسانی موجودی در جدول balance
            cursor.execute("""
