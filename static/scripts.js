async function fetchMode() {
  ...
  async function loadChart() {
    ...
    const buys = tradeData.filter(t => t.action === "buy");
    const sells = tradeData.filter(t => t.action === "sell");
  
    const buySignals = {
      x: buys.map(t => new Date(t.candle_time * 1000)),
    ...
    const sellSignals = {
      x: sells.map(t => new Date(t.candle_time * 1000)),
